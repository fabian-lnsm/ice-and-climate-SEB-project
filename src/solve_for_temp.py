...
if station == "S5" or station == "S6" or station == "S9" or station == "S10":
    SEBdata = SEBf.SEB_data(
        FileName="Pangaea-Data/GRL_" + station + "_" + dtime + "_all.csv"
    )
elif (
    station == "AWS14" or station == "AWS15" or station == "AWS17" or station == "AWS18"
):
    SEBdata = SEBf.SEB_data(
        FileName="Pangaea-Data/ANT_" + station + "_" + dtime + "_all.csv"
    )
...
Residu = (
    SWnet + LWnet + SHF + LHF + Gs - MeltS
)  # residu combines melt with non closure of energy terms
...
c_s = SHF / (
    ff10m * (T2m - Ts_mod) + 1e-6
)  # correction factor of 1e-6 to avoid division by zero
...
def SEB_equation(
    ...
    RHS = (
        ...
        - SWup
        + LWdown
        - sigma * (Ts_mod + 273.16) ** 4
        + c_s * ff10m * (T2m - Ts_mod)
        + LHF
        + Gs
    )
    return RHS - LHS
...
for i in range(len(SWdown)):
    if (
        np.isnan(SWdown[i])
        or np.isnan(SWup[i])
        or np.isnan(LWdown[i])
        or np.isnan(ff10m[i])
        or np.isnan(T2m[i])
        or np.isnan(LHF[i])
        or np.isnan(Gs[i])
        or np.isnan(MeltS[i])
        or np.isnan(Residu[i])
        or np.isnan(c_s[i])
    ):
...
if timestep == "daily":
    ...
    Time_range = [
        dt.datetime.fromisoformat("2012-01-01"),
        dt.datetime.fromisoformat("2013-01-01"),
    ]
    Time_Label = "Date"
